...
class LoginScreen {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.authenticated) {
                    ...
                    console.log('Session found, username:', data.username, 'employeeId:', data.employeeId, 'type:', typeof data.employeeId);
    ...
    async handleLogin(e) {
        ...
        try {
            ...
            if (data.success) {
                window.currentUser = data.username;
                window.currentEmployeeId = data.employeeId;
                console.log('Login successful, username:', data.username, 'employeeId:', data.employeeId, 'type:', typeof data.employeeId);
                ...
                // リダイレクト先に遷移
                const redirectPath = fetchWithAuth.getRedirectAfterLogin();
    ...
    updateRequirementsDisplay(validation) {
        const requirements = [
            { text: '8-20文字', valid: validation.length },
            { text: '英大文字を含む', valid: validation.hasUppercase },
            { text: '英小文字を含む', valid: validation.hasLowercase },
            { text: '数字を含む', valid: validation.hasNumber },
            { text: '記号を含む', valid: validation.hasSymbol },
            { text: '同じ文字の連続3回なし', valid: validation.noConsecutive },
